...
$accountSQL = "SELECT * FROM account WHERE account_number  = 'PE62918RBS'";
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">Transactions Made</strong>
                            </div>
                                ...
                                <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    <thead>
                                        <tr>
                                            <th>Trans. ID</th>
                                            <th>Date & Time (IST)</th>
                                            <th>Receipt Account Number</th>
                                            <th>Remarks</th>
                                            <th>Debit (ZWK)</th>
                                            <th>Credit (ZWK)</th>
                                            <th>Current Balance (ZWK)</th>
                                        </tr>
                                    </thead>
                                    <?php
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <!-- LOGIN TO CHECK IF THE CURRENT ACCOUNT IS A SENDER OR RECIEVER  -->


                                            <!-- QUERY THE DEBITS TABLE -->
                                        

                                           <?php
                                            $sql_deb = "SELECT * FROM debit WHERE account_number = 'PE62918RBS'";
                                            $QUERY_DEB = mysqli_query($con, $sql_deb);
                                            if (($numRows = mysqli_num_rows($QUERY_DEB)) > 0) {
                                                # code...
                                                while($rows = mysqli_fetch_assoc($QUERY_DEB)){
                                                    ?><td><?php echo number_format($row["transaction_amount"]); ?></td><?php
                                                }
                                            } else {
                                               echo " <td>0</td>";
                                            }
                                           ?>
                                           <!-- QUERY THE CREDITS TABLE -->
                                        
                                            <?php
                                            $sql_cre = "SELECT * FROM credit WHERE account_number = 'PE62918RBS'";
                                            $QUERY_CRE = mysqli_query($con, $sql_cre);
                                            if (($numRows = mysqli_num_rows($QUERY_CRE)) > 0) {
                                                # code...
                                                while($rows = mysqli_fetch_assoc($QUERY_CRE)){
                                                    ?><td><?php echo number_format($row["transaction_amount"]); ?></td><?php
                                                }
                                            } else {
                                               echo " <td>0</td>";
                                           ...
                                           ?>
                                            ...
                                            <td><?php echo number_format((float)$balance); ?></td>
